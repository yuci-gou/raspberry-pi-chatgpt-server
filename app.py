...
try:
    ...
    GPIO_AVAILABLE = False
    print(f"❌ Unexpected error loading MCP GPIO client: {e}")
...
def execute_gpio_command(pin: int, state: str) -> dict:
    """Execute GPIO command using MCP GPIO client
...
def read_gpio_command(pin: int) -> dict:
    """Read GPIO pin using MCP GPIO client
...
def get_gpio_status_command() -> dict:
    """Get GPIO status using MCP GPIO client
